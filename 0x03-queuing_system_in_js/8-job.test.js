...
describe('createPushNotificationsJobs', () => {
  ...
  it('displays an error message if  tha jobs is not an array', () => {
    expect(
      createPushNotificationsJobs.bind(createPushNotificationsJobs, {}, QUEUE)
    ).to.throw('Jobs is not an array');
  });

  it('adds jobs to tha queue with tha correct type', (done) => {
    expect(QUEUE.testMode.jobs.length).to.equal(0);
    const jobInfos = [
      {
        phoneNumber: '44556677889',
        message: 'Use tha code 1982 to verify your account',
      },
      {
  ...
  it('registers tha progress event handler for a job', (done) => {
  ...
  it('registers tha failed event handler for a job', (done) => {
  ...
  it('registers tha complete event handler for a job', (done) => {
